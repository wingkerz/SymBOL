...
from screen_pretrain_knowledge_eq import make_pop_from_file, evolve, multi_processing_eval_pop, \
    ...
    opt_constant_multi_core, multi_processing_opt_constant_SR,get_parser
...
# parser.add_argument('--case_name', help='case_name, default:"II.6.15a_5_0"', default="II.11.17_0_0")
# parser.add_argument('--dim', help='dim, default:3', default=6)
# parser.add_argument('--add_str', help='add_str, default:""', default="_iter0")
    ...
    def append(self, f_eq_str_list, g_eq_str_list, fitness_list):
        ...
        for ii in valid_indices:
            ...
            if len(self.pop) > 2500:
                ...
                del self.pop[max_index]
                del self.labels[max_index]
        print(f"成功入库: {len(valid_indices) - count_timeout - count_parse_error}")
    def str_from(self, ind_in_pop_f, simplify_flag=False, timeout_seconds=2):
        ...
        if simplify_flag:
            ...
            try:
                ...
                print(f"[Timeout] 化简超时: {raw_str}")
                return raw_str
            except Exception as e:
                print(f"[Error] 化简出错: {e}")
                return raw_str
            finally:
                signal.alarm(0)
        else:
...
def make_passages(pop_str):
    ...
    for ii in range(len(pop_str)):
        ...
        passage = 'fx = %s' % (replace_C_to_one(replace_numbers(f_eq_str)))
...
def check_operators(expression):
    ...
    if illegal_operators:
        return False, f"illegal: {', '.join(illegal_operators)}"
    return True, "legal"
    ...
    while i < len(expression):
        if expression[i].isdigit() or (
            ...
            while i < len(expression) and (expression[i].isdigit() or expression[i] == '.'):
                i += 1
            if num_start > 0 and (expression[num_start - 1] == 'x' or expression[num_start - 1] == '^' or (expression[num_start - 2:num_start] == '**')):
...
def replace_C_to_one(input_string):
    ...
    while i < len(input_string):
        ...
        if char == 'C':
            if i > 0 and (input_string[i - 1] == '^' or (i > 1 and input_string[i - 2:i] == '**')):
                result += str(random.choice([-1, 2, 3]))
            else:
                result += str(1)
        else:
            result += char
    ...
    obs = Observations()
    result = add_parentheses_around_denominator(result)
    result = obs.str_from(result, simplify_flag=True)
    result = add_parentheses_around_denominator(result)
...
def gen_use_llm_separate(pop, list_f, list_g, suggest_list, selected_pop, context_template, add_bool,variables):
    ...
    prompts_f = use_gpt.read_prompt_from_file(context).format(fx_list = fx_list, variables = variables,suggest_text=suggest_text,suggest_list = formatted_suggestion_list)
    if context[-5] == '4':
       context = context[:-5] + '2' + context[-4:]
    if context[-5] == '1':
       context = context[:-5] + '3' + context[-4:]
    prompts_g = use_gpt.read_prompt_from_file(context).format(fx_list = fx_list, variables = variables,suggest_text=suggest_text,suggest_list = formatted_suggestion_list)
    num = 10
    if context[-5] == '3':
        num = 1
    for j in range(num):
        ...
        for key in common_keys:
            try:
                ...
                if gx not in processed_list_g:
                    gx = replace_C(gx)
                    fx_list_all.append(gx)
            except Exception as e:
                print(f"Error processing key aaa {key}: fx={fx}. Error: {str(e)}")
                continue
...
def plot_validation_results(results):
    ...
    plt.close()
def bo_one_iter(surrogate, observations, list_f, list_g, suggest_list, selected_pop, prompts_path, train_num, train_flag=True,variables = ['x0']):
    if train_flag and train_num % 5 == 0:
        print("train!")
    ...
    generated_new_pop = gen_use_llm_separate(pop=observations,list_f = list_f,list_g = list_g,suggest_list = suggest_list,selected_pop=selected_pop, context_template = prompts_path,add_bool=train_num,variables = variables)
    fx_list = []
    gx_list = []
    passage = make_passages(observations.get_pop_str(generated_new_pop, simplify_flag=True))
    loss, output_mean_all, output_std_all, target_true = surrogate.evaluate_and_predict(make_passages(observations.get_pop_str(generated_new_pop, simplify_flag=True)), None)
...
def plot_surrogate_prediction_vs_true(surrogate, selected_pop, selected_fitness_list, observations, step_i,
    ...
    train_passages = make_passages(observations.get_pop_str(observations.pop, simplify_flag=True))
    train_labels = observations.labels
    loss_train, train_pred_mean, train_pred_std, train_true_labels = surrogate.evaluate_and_predict(train_passages,
                                                                                                    train_labels)
    ...
    print(f"Pearson: {train_corr:.3f} (p={train_p:.3g})")
    test_corr, test_p = pearsonr(test_true, test_pred)
    print(f"Pearson: {test_corr:.3f} (p={test_p:.3g})")
    os.makedirs(save_dir, exist_ok=True)
    plt.rcParams.update({
        'font.size': 15,          
        'axes.titlesize': 15,     
    ...
    plt.text(
    x=0.05, y=0.95,
    s=f"r_train : {train_corr:.3f}\nr_test : {test_corr:.3f}",
    transform=plt.gca().transAxes,
    ha='left', va='top',
    fontsize=20,
    bbox=dict(facecolor='white', alpha=0.8)
)
    plt.errorbar(train_true, train_pred, yerr=2 * train_std, fmt='o',
                 color='blue', alpha=0.5, label='Training Set Predictions (mean±2σ)')
    plt.errorbar(test_true, test_pred, yerr=2 * test_std, fmt='o',
                 color='red', alpha=0.7, label='Test Set Predictions(mean±2σ)')
    min_val = min(min(train_true), min(test_true), min(train_pred), min(test_pred))
    max_val = max(max(train_true), max(test_true), max(train_pred), max(test_pred))
    ...
    with open(save_path, "a", encoding="utf-8") as fw:
        ...
        fw.write("=========================================================\n")
def evaluate_surrogate_with_increasing_data_from_observations(
    surrogate,
    observations,
    save_dir,
    step_name="surrogate_eval"
):
    import os
    import random
    import numpy as np
    ...
    from scipy.stats import pearsonr
    os.makedirs(save_dir, exist_ok=True)
    all_pop = observations.pop
    if len(all_pop) < 300:
        raise ValueError(f"STORED_OBSERVATIONS 中个体不足 300 个，仅有 {len(all_pop)} 个")
    selected_pop = all_pop[:300]
    selected_pop_str = observations.get_pop_str(selected_pop, simplify_flag=True)
    selected_fitness_list = [p_str[1] for p_str in selected_pop_str]
    indices = list(range(len(selected_pop)))
    random.seed(42)
    random.shuffle(indices)
    train_indices = indices[:150]
    test_indices = indices[200:300]
    train_pop_str = [selected_pop_str[i] for i in train_indices]
    test_pop_str = [selected_pop_str[i] for i in test_indices]
    train_labels = [selected_fitness_list[i] for i in train_indices]
    test_labels = [selected_fitness_list[i] for i in test_indices]
    test_passages = make_passages(test_pop_str)
    ...
    for i, train_size in enumerate(range(10, 151, 10)):
        sub_indices = random.sample(range(150), train_size)
        sub_train_pop_str = [train_pop_str[j] for j in sub_indices]
        sub_train_labels = [train_labels[j] for j in sub_indices]
        train_passages = make_passages(sub_train_pop_str)
        train_labels_tensor = torch.tensor(sub_train_labels).float()
        print(f"\nStep {i + 1}: Training with {train_size} samples")
        surrogate.train(train_passages, train_labels_tensor)
        print("Train complete.")
        loss_test, pred_mean_test, pred_std_test, true_labels_test = surrogate.evaluate_and_predict(
            test_passages, test_labels_tensor)
        pred_test = pred_mean_test.cpu().numpy().flatten()
        std_test = pred_std_test.cpu().numpy().flatten()
        true_test = true_labels_test.cpu().numpy().flatten()
        test_corr, test_p = pearsonr(true_test, pred_test)
        loss_train, pred_mean_train, pred_std_train, true_labels_train = surrogate.evaluate_and_predict(
            train_passages, train_labels_tensor)
        pred_train = pred_mean_train.cpu().numpy().flatten()
        std_train = pred_std_train.cpu().numpy().flatten()
        true_train = true_labels_train.cpu().numpy().flatten()
        train_corr, train_p = pearsonr(true_train, pred_train)
        print(f"Train r: {train_corr:.3f} | Test r: {test_corr:.3f}")
        plt.figure(figsize=(9, 9))
        plt.errorbar(true_train, pred_train, yerr=2 * std_train, fmt='o', color='#4682B4', alpha=0.7,label='Train Predictions ±2σ')
        plt.errorbar(true_test, pred_test, yerr=2 * std_test, fmt='o', color='#CD5C5C', alpha=0.7,label='Test Predictions ±2σ')
        min_val = min(true_train.min(), pred_train.min(), true_test.min(), pred_test.min())
        max_val = max(true_train.max(), pred_train.max(), true_test.max(), pred_test.max())
        plt.plot([min_val, max_val], [min_val, max_val], 'k--', lw=2, label='Perfect Prediction')
        legend = plt.legend(loc='upper right', bbox_to_anchor=(1.0, 1.0),
                        frameon=True, framealpha=0.7, edgecolor='black')
        legend.get_frame().set_linewidth(2)  
        ...
        for axis in ['top', 'bottom', 'left', 'right']:
            plt.gca().spines[axis].set_linewidth(2)
        save_path = os.path.join(save_dir, f"{step_name}_step{i + 1}_n{train_size}.png")
...
def perform_bo_iteration(num_iter=100):
    ...
    prompts_path = "/home/name/SymBOL_Code/prompts/SR/gen_prompts_1.txt"
    generated_init_pop = gen_use_llm_separate(pop=STORED_OBSERVATIONS, list_f=[], list_g=[], suggest_list= [],selected_pop= [],context_template=prompts_path,add_bool=0,variables = variables)
    selected_pop = multi_processing_opt_constant_SR(list(reversed(generated_init_pop)),
    ...
    for step_i in range(num_iter):
        sorted_pop = sorted(STORED_OBSERVATIONS.get_pop_str(STORED_OBSERVATIONS.pop), key=lambda p_str: p_str[1])[:100]
        list_f = [replace_C_to_one(replace_numbers(p_str[0][0])) for p_str in sorted_pop]
        list_g = [replace_C_to_one(replace_numbers(p_str[0][1])) for p_str in sorted_pop]
        ...
        if step_i > 0 and step_i % 5 ==0:
            prompts_path = "/home/name/SymBOL_Code/prompts/SR/gen_prompts_7.txt"
            suggest_list = suggest_use_llm(list_f = list_f,list_g = list_g, context_template = prompts_path)
        prompts_path = "/home/name//SymBOL_Code/prompts/SR/gen_prompts_2.txt"
        if step_i > 0:
            sorted_pop = selected_pop_str
        selected_pop = bo_one_iter(deepsurr, STORED_OBSERVATIONS, list_f, list_g, suggest_list, sorted_pop,prompts_path, step_i, train_flag=True,variables = variables)
        selected_pop = multi_processing_opt_constant_SR(list(reversed(selected_pop)),
